...
class TaskListener(TaskConfig):
    ...
    async def clean(self):
        try:
            ...
            await gather(
                ...
                self.MessageUtils.deleteStatus()
            )
        except:
            pass
    ...
    async def on_download_start(self):
        ...
        if Config.DELETE_LINK:
            await self.MessageUtils.delete(reply_to=True, wait=True)
    ...
    async def on_download_complete(self):
        ...
        if self.is_leech:
            ...
            await gather(
                self.MessageUtils.updateStatus(),
                tg.upload(),
            )
            del tg
    ...
    async def on_upload_complete(
        ...
        if self.is_leech:
            ...
            msg += f"\n<b>cc: </b>{self.tag}\n\n"
            if not files:
                await self.MessageUtils.reply(msg)
            else:
                fmsg = ""
                for index, (link, name) in enumerate(files.items(), start=1):
                    fmsg += f"{index}. <a href='{link}'>{name}</a>\n"
                    if len(fmsg.encode() + msg.encode()) > 4000:
                        await self.MessageUtils.reply(msg + fmsg)
                        await sleep(1)
                        fmsg = ""
                if fmsg != "":
                    await self.MessageUtils.reply(msg + fmsg)
            ...
            if (
                ...
                button = None
            msg += f"\n\n<b>cc: </b>{self.tag}"
            await self.MessageUtils.reply(msg, button)
        if self.seed:
        ...
        if count == 0:
            ...
            await self.MessageUtils.updateStatus()
    ...
    async def on_upload_error(self, error):
        async with task_dict_lock:
            if self.mid in task_dict:
                del task_dict[self.mid]
            count = len(task_dict)
        await self.MessageUtils.reply(f"{self.tag} {escape(str(error))}")
        if count == 0:
            await self.clean()
        else:
            await self.MessageUtils.updateStatus()
