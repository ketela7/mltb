...
async def path_updates(_, query, obj):
    ...
    if data[1] == "cancel":
        ...
        obj.event.set()
        await obj.listener.MessageUtils.delete(message)
        return
    if obj.query_proc:
        return
    obj.query_proc = True
...
class RcloneList:
    ...
    async def _send_list_message(self, msg, button):
        if not self.listener.is_cancelled:
            if self._reply_to is None:
                self._reply_to = await self.listener.MessageUtils.reoly(msg, button)
            else:
                await self.listener.MessageUtils.edit(msg, button, message=self._reply_to)
    ...
    async def get_path_buttons(self):
        ...
        page = (self.iter_start / LIST_LIMIT) + \
            1 if self.iter_start != 0 else 1
        buttons = ButtonMaker()
        ...
        if self.list_status == "rcu" or len(self.path_list) > 0:
            buttons.data_button("Choose Current Path",
        ...
        if len(self.selected_pathes) > 1:
            buttons.data_button("Done With Selection",
                                "rcq ds", position="footer")
            buttons.data_button("Clear Selection",
                                "rcq clear", position="footer")
        if self.list_status == "rcu":
            buttons.data_button("Set as Default Path",
    ...
    async def get_rclone_path(self, status, config_path=None):
        ...
        if config_path is None:
            self._rc_user, self._rc_owner = await gather(
                aiopath.exists(self.user_rcc_path), aiopath.exists(
                    "rclone.conf")
            )
