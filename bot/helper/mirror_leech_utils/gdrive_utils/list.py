...
async def id_updates(_, query, obj):
    ...
    if data[1] == "cancel":
        ...
        obj.event.set()
        await obj.listener.MessageUtils.delete()
        return
    if obj.query_proc:
        return
    obj.query_proc = True
...
class GoogleDriveList(GoogleDriveHelper):
    ...
    async def _send_list_message(self, msg, button):
        if not self.listener.is_cancelled:
            if self._reply_to is None:
                self._reply_to = await self.listener.reply(msg, button)
            else:
                await self.listener.edit(msg, button, message=self._reply_to)
    ...
    async def get_items_buttons(self):
        ...
        if self.list_status == "gdu":
            default_id = self.listener.user_dict.get(
