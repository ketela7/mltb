...
class BotCommands:
    ...
    YtdlCommand = [f"ymirror{i}", f"ym{i}"]
    ...
    YtdlLeechCommand = [f"yleech{i}", f"yl{i}"]
    ...
    HelpCommand = [f"help{i}", f"h{i}"]
    LogCommand = f"log{i}"
    ShellCommand = [f"shell{i}", f"sh{i}"]
...
cmds = sorted([
    BotCommand(v[0], f"or /{v[1]}" if len(v) > 1 else "~")
    if isinstance(v, list) else BotCommand(v, "~")
    for k, v in vars(BotCommands).items()
