...
class TaskConfig:
    def __init__(self):
        ...
        self.is_super_chat = self.message.chat.type.name in [
            "SUPERGROUP", "CHANNEL"]
        self.hashtag = ""
        self.MessageUtils = MessageUtils(self.message, self.client)
    ...
    def get_config_path(self, dest):
        return (
            f"rclone/{self.user_id}.conf" if dest.startswith(
                "mrcc:") else "rclone.conf"
        )
    ...
    async def before_start(self):
        ...
        if not self.is_leech:
            ...
            default_upload = (
                self.user_dict.get(
                    "DEFAULT_UPLOAD", "") or Config.DEFAULT_UPLOAD
            )
            if (not self.up_dest and default_upload == "rc") or self.up_dest == "rc":
                self.up_dest = self.user_dict.get(
                    "RCLONE_PATH") or Config.RCLONE_PATH
            elif (not self.up_dest and default_upload == "gd") or self.up_dest == "gd":
                self.up_dest = self.user_dict.get(
                    "GDRIVE_ID") or Config.GDRIVE_ID
            if not self.up_dest:
    ...
    async def run_multi(self, input_list, obj):
        ...
        if self.multi_tag and self.multi_tag not in multi_tags:
            await self.MessageUtils.reply(
                f"{self.tag} Multi Task has been cancelled!"
            )
            await self.MessageUtils.sendStatus()
        ...
        if len(self.bulk) != 0:
            ...
            nextmsg = await self.MessageUtils.reply(msgts)
        else:
            ...
            if self.multi > 2:
                msgts += f"\nCancel Multi: <code>/{BotCommands.CancelTaskCommand[1]} {self.multi_tag}</code>"
            nextmsg = await self.MessageUtils.reoly(msgts, message=nextmsg)
        nextmsg = await self.client.get_messages(
    ...
    async def init_bulk(self, input_list, bulk_start, bulk_end, obj):
        try:
            ...
            if len(self.bulk) > 2:
                ...
                multi_tags.add(self.multi_tag)
                msg += f"\nCancel Multi: <code>/{BotCommands.CancelTaskCommand[1]} {self.multi_tag}</code>"
            nextmsg = await self.MessageUtils.reply(msg)
            nextmsg = await self.client.get_messages(
                chat_id=self.message.chat.id, message_ids=nextmsg.id
            )
            if self.message.from_user:
            ...
            await self.MessageUtils.reply(
                f"Reply to text file or to telegram message that have links separated by new line!\nERROR: {e}",
            )
    ...
    async def generate_screenshots(self, dl_path):
        ss_nb = int(self.screen_shots) if isinstance(
            self.screen_shots, str) else 10
        if self.is_file:
    ...
    async def generate_sample_video(self, dl_path, gid):
        data = (
            self.sample_video.split(":") if isinstance(
                self.sample_video, str) else ""
        )
        if data:
    ...
    async def proceed_split(self, dl_path, gid):
        ...
        if self.is_file:
            ...
            if f_size > self.split_size:
                self.files_to_proceed[dl_path] = [
                    f_size, ospath.basename(dl_path)]
        else:
            for dirpath, _, files in await sync_to_async(walk, dl_path, topdown=False):
                for file_ in files:
                    f_path = ospath.join(dirpath, file_)
