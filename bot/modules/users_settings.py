...
async def add_one(_, message, option):
    ...
    if value.startswith("{") and value.endswith("}"):
        ...
        await mu.reply("It must be dict!")
        return
    await mu.delete()
...
async def get_menu(option, message, user_id):
    ...
    await mu.edit(text, buttons.build_menu(2))
...
async def ffmpeg_variables(
    ...
    if ffc:
        ...
        if key is None:
            ...
            return
        await mu.edit(msg, buttons.build_menu(2))
...
async def edit_user_settings(client, query):
    ...
    if user_id != int(data[1]):
        ...
        buttons.data_button("Back", f"userset {user_id} setevent")
        buttons.data_button("Close", f"userset {user_id} close")
        await mu.edit(text, buttons.build_menu(1))
        ...
        await query.answer()
        if data[3] == "THUMBNAIL":
            await mu.reply(name,document=thumb_path)
