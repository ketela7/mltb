...
async def torrent_search(_, message):
    ...
    if SITES is None and not Config.SEARCH_PLUGINS:
        await mu.reply("No API link or search PLUGINS added for this function"
                       )
    elif len(key) == 1 and SITES is None:
        await mu.reply("Send a search key along with command")
    elif len(key) == 1:
        buttons.data_button("Trending", f"torser {user_id} apitrend")
        buttons.data_button("Recent", f"torser {user_id} apirecent")
        buttons.data_button("Cancel", f"torser {user_id} cancel")
        button = buttons.build_menu(2)
        await mu.reply("Send a search key along with command", button)
        ...
        buttons.data_button("Cancel", f"torser {user_id} cancel")
        button = buttons.build_menu(2)
        await mu.reply("Choose tool to search:", button)
    elif SITES is not None:
        button = api_buttons(user_id, "apisearch")
        await mu.reply("Choose site to search | API:", button)
    else:
        button = await plugin_buttons(user_id)
        await mu.reply("Choose site to search | Plugins:", button)
...
async def torrent_search_update(_, query):
    ...
    if user_id != int(data[1]):
        ...
        await mu.edit("Choose site:", button)
    elif data[2] == "plugin":
        await query.answer()
        button = await plugin_buttons(user_id)
        await mu.edit("Choose site:", button)
        ...
        await query.answer()
        await mu.edit("Search has been canceled!")
