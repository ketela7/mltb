...
async def task_status(_, message):
    ...
    if count == 0:
        ...
        msg = f"No Active Tasks!\nEach user can get status for his tasks by adding me or user_id after cmd: /{BotCommands.StatusCommand[0]} me"
        msg += (
            f"\n<b>CPU:</b> N/A% | <b>FREE:</b> {free}"
            f"\n<b>RAM:</b> {virtual_memory().percent}% | <b>UPTIME:</b> {currentTime}"
        )
        reply_message = await mu.reply(msg)
        await mu.delete(reply_message, True, True)
    else:
...
async def status_pages(_, query):
    ...
    await query.answer()
    if data[2] == "ref":
        await mu.updateStatus(key, force=True)
...
"""
        ...
        await mu.edit(msg, button.build_menu())
