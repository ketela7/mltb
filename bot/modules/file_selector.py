...
async def select(_, message):
    ...
    if not Config.BASE_URL:
        await mu.reply("Base URL not defined!")
        return
    ...
    if not iscoroutinefunction(task.status):
        await mu.reply("The task have finshed the download stage!")
        return
    ...
    if task.name().startswith("[METADATA]") or task.name().startswith("Trying"):
        await mu.reply("Try after downloading metadata finished!")
        return
