...
async def select_format(_, query, obj):
    ...
    if data[1] == "dict":
        ...
        await obj.listener.MessageUtils.edit("Task has been cancelled.")
        obj.qual = None
...
class YtSelection:
    ...
    async def _event_handler(self):
        ...
        try:
            ...
            await self.listener.MessageUtils.edit("Timed Out. Task has been cancelled!", message=self._reply_to)
            self.qual = None
    ...
    async def get_quality(self, result):
        ...
        self._reply_to = await self.listener.MessageUtils.reply(msg, self._main_buttons
        )
        await self._event_handler()
        if not self.listener.is_cancelled:
            await self.listener.MessageUtils.delete(self._reply_to)
        return self.qual
    ...
    async def back_to_main(self):
        if self._is_playlist:
            ...
            msg = f"Choose Video Quality:\nTimeout: {get_readable_time(self._timeout - (time() - self._time))}"
        await self.listener.MessageUtils.edit(msg, self._main_buttons, message=self._reply_to)
    ...
    async def qual_subbuttons(self, b_name):
        ...
        await self.listener.MessageUtils.edit(msg, subbuttons, message=self._reply_to)
    ...
    async def new_event(self):
        ...
        if not is_url(self.link):
            await self.MessageUtils.reply(
                 COMMAND_USAGE["yt"][0], COMMAND_USAGE["yt"][1]
            )
        ...
        try:
            ...
            await self.MessageUtils.reply(e)
            await self.remove_from_same_dir()
            return
        ...
        try:
            ...
            await self.MessageUtils.reply(f"{self.tag} {msg}")
            await self.remove_from_same_dir()
            return
        finally:
