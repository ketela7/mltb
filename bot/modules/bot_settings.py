...
async def update_buttons(message, key=None, edit_type=None):
    ...
    await MessageUtils(message).edit(msg, button)
...
async def edit_aria(_, message, pre_message, key):
    ...
    await MessageUtils(message).delete()
...
async def update_private_file(_, message, pre_message):
    ...
    if not message.media and (file_name := message.text):
        ...
        if "@github.com" in Config.UPSTREAM_REPO:
            ...
            await mu.reply(msg, buttons.build_menu(2))
        else:
            await mu.delete()
...
async def edit_bot_settings(client, query):
    ...
    if data[1] == "close":
        ...
        await mu.delete(message.reply_to_message)
        await mu.delete(message)
...
async def load_config():
    ...
    if len(task_dict) != 0 and (st := intervals["status"]):
        for key, intvl in list(st.items()):
            intvl.cancel()
            intervals["status"][key] = SetInterval(
                Config.STATUS_UPDATE_INTERVAL, mu.updateStatus, key
            )
