...
async def _list_drive(key, message, item_type, is_recursive, user_token, user_id):
    ...
    if telegraph_content:
        ...
        await mu.edit(f"No result found for <i>{key}</i>")
...
async def select_type(_, query):
    ...
    if user_id != int(data[1]):
        ...
        return await mu.edit("Choose list options:", buttons)
    elif data[2] == "ut":
        await query.answer()
        ...
        await query.answer()
        return await mu.edit("list has been canceled!")
    await query.answer()
    item_type = data[2]
    is_recursive = eval(data[3])
    user_token = eval(data[4])
    await mu.edit(f"<b>Searching for <i>{key}</i></b>")
...
async def gdrive_search(_, message):
    ...
    if len(message.text.split()) == 1:
        return await mu.reply("Send a search key along with command")
    user_id = message.from_user.id
    buttons = await list_buttons(user_id)
    await mu.reply("Choose list options:", buttons)
